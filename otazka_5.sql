...
CREATE OR REPLACE VIEW v_gdp_payroll_food_price_comparison AS
SELECT
    food_price_trend.year,
    ROUND(AVG(food_price_trend.percent_change), 2) AS avg_food_price_percent_change,  
    price_payroll.national_payroll_percent_change AS national_payroll_percent_change,  
    gdp.gdp_percent_change AS gdp_percent_change
FROM v_food_price_trend_analysis AS food_price_trend
JOIN v_price_vs_payroll_comparison AS price_payroll
    ON food_price_trend.year = price_payroll.year
JOIN (
    SELECT 
        e.year,
        e.gdp,
    ...
    ON food_price_trend.year = gdp.year
GROUP BY food_price_trend.year, price_payroll.national_payroll_percent_change, gdp.gdp_percent_change
ORDER BY food_price_trend.year;
...
FROM v_gdp_payroll_food_price_comparison
WHERE year > 2006
ORDER BY year;
...
FROM v_gdp_payroll_food_price_comparison AS gdp_payroll_price
WHERE year > 2006
ORDER BY year;
